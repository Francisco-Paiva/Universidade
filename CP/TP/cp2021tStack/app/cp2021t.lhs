...
\begin{code}
g_eval_exp a = either (const a) num where
    num = either (id) nam
    nam = either (uncurry bin) (uncurry un)
    bin Sum = uncurry (+)
    bin Product = uncurry (*)
    un Negate = negate
    un E = expd
...
\begin{code}
gopt a = (either (q1) (either q2 (either q3 q4))) where
    q1 = const a
    q2 = id
    q3 = uncurry bin
    bin Sum = uncurry (+)
...
sd_gen = (either q1 (either q2 (either q3 q4))) where
    q1 x = (X,N 1)
    q2 a = (N a,N 0)
    q3 (Sum,((a,b),(x,y))) = (Bin Sum a x,Bin Sum b y)
    q3 (Product,((a,b),(x,y))) = (Bin Product a x, Bin Sum (Bin Product a y) (Bin Product b x))
    q4 (Negate,(x,y)) = (Un Negate x,Un Negate y)
    q4 (E,(x,y)) = (Un E x,Bin Product (Un E x) y)
\end{code}
...
\begin{code}
ad_gen v = (either q1 (either q2 (either q3 q4))) where
    q1 x = (v,1)
    q2 a = (a,0)
    q3 (Sum,((a,b),(x,y))) = (a+x,b+y)
    q3 (Product,((a,b),(x,y))) = (a*x,(a*y)+(b*x))
    q4 (Negate,(x,y)) = ((negate x),(negate y))
    q4 (E,(x,y)) = (expd x,(expd x)*y)
\end{code}
...
\begin{code}
avg_aux = cataV (either b q) where
    b = split id (const 1)
    q = split q1 q2
    q1(a,(med,len)) = (a + len*med) / (1+len)
    q2 = succ.p2.p2
\end{code}
...
avgLTree = p1.cataLTree gene where
    gene = either b q where
        b = split id (const 1)
        q = split q1 q2
        q1((med1,len1),(med2,len2)) = ((med1*len1) + (med2*len2)) / (len1 + len2)
        q2 = (uncurry (+)).(p2 >< p2)
\end{code}
